  ...
  For i = 1 to ws(2).Cells(1,1).End(xlDown).Row
    ...
    If ws(2).Cells(i,5) = 1 Then
      ...
      Do
        start1 = 0
        For j = 1 to keta
            start1 = start1 + check(j) * 1/(10 ^ j)
        Next j
        start2 = start1 + 1/(10 ^ keta)
        ...
        If InStr(objIE.document.all(0).outerHTML,">全0件</span>") <> 0 Then
          Debug.Print "under0"
          If check(keta) = 9 Then
            Do while check(keta) = 9
              check(keta) = 0
              keta = keta - 1
            Loop
          End If
          check(keta) = check(keta) + 1
        ...
        Else
          ...
          If IsNull(check(keta)) = True Then
            check(keta) = 1
          ElseIf check(keta) < 9  Then
            check(keta) = check(keta) + 1
          ElseIf check(keta) = 9 Then
            Do while check(keta) = 9
              check(keta) = 0
